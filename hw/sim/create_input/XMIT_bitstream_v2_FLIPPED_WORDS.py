...
def main():
	...
	#file opened above. 
		with open(options.filename,"rb+") as infile:
			...
			for row in reader:
				for word in row:
					#Header words are packed as distinct 32 bit words FFFFFFFF or E0000000
					if word.upper() == "E0000000":
                                                word1=word[0:4]
                                                word2=word[4:]
                                                outfile.write("\""+format(int(word1,16), '0>16b')+format(int(word2,16), '0>16b')+"\",\n")
					#Data words are packed in 32 bit words as two separate 16 bit words, the first being the rightmost 16 bit word
					elif len(word) == 8:
						word2=word[0:4]
						word1=word[4:]
						outfile.write("\""+format(int(word1,16), '0>16b')+format(int(word2,16), '0>16b')+"\",\n")
					elif len(word) !=8 and len(word)>0:
						print "ERROR: Input included word which wasn't 32 bits"
